...
public class GetLocationsCM {
    String apiresponse = "";


    public GetLocationsCM(String baseurl, GetLocationsRequest request) {


        //RequestQueue is managed by SDK Communications Manager

        String url = baseurl + R.string.getlocationsapi + (request.ll == null ? "" : "?ll=" + request.ll);
        Log.d("GetLocCM", url);
    ...
    public String GetLocationList(String baseurl, GetLocationsRequest request) {
        if (apiresponse.length() > 1) {
            {
                new GetLocationsCM(baseurl, request);
            }
